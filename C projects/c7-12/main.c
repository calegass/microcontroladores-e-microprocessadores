      ...
      if (setting_time) {
        if (showing_alarm) {
          ...
          if (!input(S4)) {
            ...
            case 3:
              alarm_days++;
              if (alarm_months == 1 || alarm_months == 3 || alarm_months == 5 ||
                  alarm_months == 7 || alarm_months == 8 ||
                  alarm_months == 10 || alarm_months == 12) {
                if (alarm_days == 32) {
                  alarm_days = 1;
                }
              } else if (alarm_months == 4 || alarm_months == 6 ||
                         alarm_months == 9 || alarm_months == 11) {
                if (alarm_days == 31) {
                  alarm_days = 1;
                }
...
void display_alarm() {
  lcd_gotoxy(1, 1);
  printf(lcd_write_dat, "ALARM: %02u:%02u:%02u ", alarm_hours, alarm_minutes,
         alarm_seconds);
  lcd_gotoxy(1, 2);
  printf(lcd_write_dat, "     %02d/%02d/%04LU ", alarm_days, alarm_months,
         alarm_years);
...
void alarm() {
  if (hours == alarm_hours && minutes == alarm_minutes &&
      seconds == alarm_seconds && days == alarm_days &&
      months == alarm_months && years == alarm_years) {
    ringing = 1;
    output_high(BUZZER);
  }
}